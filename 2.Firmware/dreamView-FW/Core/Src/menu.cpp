...
//列表变量
#define   LIST_FONT           u8g2_font_myfont
#define   LIST_NUM            100
...
//弹窗变量
#define   WIN_FONT            u8g2_font_myfont
#define   WIN_H               27
#define   WIN_W               90
...
void knob_inter()
{
  btn.alv = HAL_GPIO_ReadPin(KEY1_GPIO_Port, KEY1_Pin);
  btn.blv = HAL_GPIO_ReadPin(KEY2_GPIO_Port, KEY2_Pin);
...
void btn_scan()
{
  btn.val = HAL_GPIO_ReadPin(KEY1_GPIO_Port, KEY1_Pin);
  if (btn.val != btn.val_last)
  {
    btn.val_last = btn.val;
    HAL_Delay(ui.param[BTN_SPT]);
    btn.val = HAL_GPIO_ReadPin(KEY1_GPIO_Port, KEY1_Pin);
    if (btn.val == 0)
    {
      btn.pressed = true;
...
{
  const char *p = (const char *) (char *) check_box.v[n - 1];
  u8g2_DrawStr(&u8g2, list.text_w_temp, LIST_TEXT_H + LIST_TEXT_S + list.text_y_temp, p);
}                 //数值
void list_draw_cbf()
{
  u8g2_DrawRFrame(&u8g2, list.text_w_temp, CB_U + list.text_y_temp, CB_W, CB_H, 0.5f);
}                                                    //外框
void list_draw_cbd()
{
  u8g2_DrawBox(&u8g2, list.text_w_temp + CB_D + 1, CB_U + CB_D + 1 + list.text_y_temp, CB_W - (CB_D + 1) * 2,
               CB_H - (CB_D + 1) * 2);
}  //外框里面的点
...
void list_show(uint8_t ui_index, struct MENU arr[])
{
  ...
  {
    ui.oper_flag = false;
    list.box_W = u8g2_GetUTF8Width(&u8g2, arr[ui.select[ui.layer]].m_select) + LIST_TEXT_S * 2;
  ...
  for (int i = 0; i < list.num[ui_index]; ++i)
  {
    ...
    const char *p = (const char *) (char *) arr[i].m_select;
    u8g2_DrawStr(&u8g2, list.text_x_temp + LIST_TEXT_S, LIST_TEXT_S + LIST_TEXT_H + list.text_y_temp, p);
  ...
  //绘制进度条和选择框
  u8g2_DrawBox(&u8g2, UI_DISP_W - LIST_BAR_W, 0, LIST_BAR_W, list.bar_h);
  u8g2_DrawRBox(&u8g2, 0, list.box_y - (list.box_h - LIST_LINE_H) / 2, list.box_w, list.box_h, LIST_BOX_R);
}
...
void win_show()
{
  ...
  u8g2_DrawBox(&u8g2, win.box_x, win.box_y, win.box_w, win.box_h);  //绘制外框背景
  u8g2_SetDrawColor(&u8g2, 2);
  u8g2_DrawFrame(&u8g2, win.box_x, win.box_y, win.box_w, win.box_h);  //绘制外框描边
  if (win.box_h > (WIN_TITLE_H + 2 * WIN_TITLE_S))
  {

    const char *p1 = (const char *) (char *) *win.value; //绘制数值
    u8g2_DrawStr(&u8g2, win.box_x + WIN_VALUE_S, win.box_y + WIN_TITLE_S + WIN_TITLE_H, p1);

    const char *p2 = (const char *) (char *) win.title; //绘制标题
    u8g2_DrawStr(&u8g2, win.box_x + WIN_TITLE_S, win.box_y + WIN_TITLE_S + WIN_TITLE_H, p2);

    u8g2_DrawBox(&u8g2, win.box_x + WIN_TITLE_S, win.box_y + win.box_h - WIN_TITLE_S - WIN_BAR_H - 1, win.bar_x,
                 WIN_BAR_H); //绘制进度条
  }
...
void f0_show()
{
  ...
  //绘制元素
  u8g2_DrawRBox(&u8g2, (int16_t) f0.box_x, (int16_t) f0.box_y, f0.box_w, f0.box_h, F0_BOX_R);
}
...
//  u8g2.enableUTF8Print();
  ...
  ui.buf_ptr = u8g2_GetBufferPtr(&u8g2);
  ui.buf_len = 8 * u8g2_GetBufferTileHeight(&u8g2) * u8g2_GetBufferTileWidth(&u8g2);
}
